  ...
  return (
    <>
      ...
      >
        ...
        <div className="align-center flex h-[45%] w-full justify-center rounded-lg border p-3">
          <Tasklayout
            title="Today"
            tasklist={tasklist.filter((tasks) => checkiftoday(tasks.date))}
          />
        </div>
        <div className="align-center flex h-[42%] w-full flex-row items-start justify-between ">
          <div className="h-full w-[49%] rounded-lg border p-3">
            <Tasklayout
              title="Tomorrow"
              tasklist={tasklist.filter((tasks) => checkiftomorrow(tasks.date))}
            />
          </div>
          <div className="h-full w-[49%] rounded-lg border p-3">
            <Tasklayout
              title="This Week"
              tasklist={tasklist.filter((tasks) => checkifinweek(tasks.date)).slice(0, 5)}
            />
          </div>
        </div>
      </div>
